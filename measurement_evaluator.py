...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='calvis', help= "nomo or calvis")
    args = parser.parse_args()
    
    template = np.load(args.path_folder.split("/")[0]+"/"+args.path_folder.split("/")[1]+'/'+args.gender+'_template.npy')
    shape_dirs = np.load(args.path_folder.split("/")[0]+"/"+args.path_folder.split("/")[1]+'/'+args.gender+'_shapedirs.npy')

    # print('shape dirs')
    # print(shape_dirs.shape)
    features_path = args.path_folder+"/features"
    dataloaders_path = args.path_folder + "/dataloaders"
    ...
    if args.dataset == 'calvis':
        X_train = np.load(f"{args.path_folder}/features/{args.gender}/ae_train_features.npy")
        X_train = X_train.squeeze()
        X_test = np.load(f"{args.path_folder}/features/{args.gender}/ae_test_features.npy")
        X_test = X_test.squeeze()
        X_train_h_w = np.load(f'{args.path_folder}/dataloaders/{args.gender}/train_h_w_measures_{args.gender}_density.npy', allow_pickle=True)
        X_test_h_w = np.load(f'{args.path_folder}/dataloaders/{args.gender}/test_h_w_measures_{args.gender}_density.npy', allow_pickle=True)
        ...
        X_train = np.load(f'{args.path_folder}/features/{args.gender}/{args.features}_train_features.npy')
        X_test = np.load(f'{args.path_folder}/{args.gender}/{args.features}_test_features.npy')
        X_train_h = np.load(f'{args.path_folder}/{args.gender}/train_heights.npy').reshape(len(X_train), 1)
        X_test_h = np.load(f'{args.path_folder}/{args.gender}/test_heights.npy').reshape(len(X_test), 1)
    ...
    dump(human, f'{args.path_folder}/features/{args.gender}/new_base_{args.gender}_krr.pkl')

    del human
    
    human = load(f'D:\\Francesco Manigrasso\\avatargenerator\\Dataset\\data{args.parameters}\\{args.split}\\features2\\{args.gender}\\new_base_{args.gender}_krr.pkl')
