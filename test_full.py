...
class AETrainer(object):
    ...
    def train_model(self,  args):
        ...
        best_models = [ 
            ...
            #"D:/Francesco Manigrasso/avatargenerator/Dataset/data128/train_test_data_fold2001_male",
        ...
        """
        #"data128","data64","data10", "data16","data32", "data64",

        Dataset_list = [ "data256", "data300"] #
        ...
        for mod in best_models:
            ...
            feature_extractor_weights = torch.load(
                path + "/weights2/0.0001/new_base_feature_extractor_" + gender + "_200.pth",
                    ...
                    try:
                        if args.model == 'ae':
                            ...
                            def create_features(loader_type, path=None):
                                ...
                                os.makedirs("features_test" + "/" + gender, exist_ok=True)
                                np.save(f"features_test/{gender}/ae_{loader_type}_features.npy",
                                        features)
                        ...
                        X_train = np.load(f"features_test/{gender}/ae_train_features.npy")
                        X_train = X_train.squeeze()
                        X_test = np.load(f"features_test/{gender}/ae_test_features.npy")
                        X_test = X_test.squeeze()
                        X_validation = np.load(f"features_test/{gender}/ae_validation_features.npy")
                        ...
                        y_shape_test = np.load(f'D:/Francesco Manigrasso/avatargenerator/Dataset/{data_iter}/{fol_iter}/dataloaders/{gender}/test_betas.npy')
                        ...
                        with open('results.csv', 'a') as out:
                            out.write(
                                mod + ";" + data_iter + ";" + fol_iter + ";validation;" + np.round(mae,
                                                                                                   5).__str__() + ";" + np.round(
                                    std,
                                    5).__str__() + ";" + np.round(
                                    mae2, 5).__str__() + ";" + np.round(std2, 5).__str__() + ";" + np.round(
                        ...
                        with open('results.csv', 'a') as out:
                            out.write(
                                mod + ";" + data_iter + ";" + fol_iter + ";test;" + np.round(mae,
                                                                                             5).__str__() + ";" + np.round(
                                    std,
                                    5).__str__() + ";" + np.round(
                                    mae2, 5).__str__() + ";" + np.round(std2, 5).__str__() + ";" + np.round(
                        ...
                        os.makedirs(f'D:/Francesco Manigrasso/avatargenerator/Dataset/objects/{"/".join(path.split("/")[2:]).replace("/", "_")}_{gender}_{data_iter}_{fol_iter}', exist_ok=True)

                        for index in tqdm(range(shape.shape[0])):
                            human.display_3D(np.array(shape[index, :]).reshape((-1, 1))).export(
                                f'D:/Francesco Manigrasso/avatargenerator/Dataset/objects/{"/".join(path.split("/")[2:]).replace("/", "_")}_{gender}_{data_iter}_{fol_iter}/mesh_{index}.obj')
