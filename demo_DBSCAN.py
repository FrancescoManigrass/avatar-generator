...
def main():
    ...
    parser.add_argument("--parameters", type=str, default='128')
    parser.add_argument("--split", type=str, default='train_test_data_fold1') #train_test_data_fold4001 uomo con 10
    ...
    try:
        os.mkdir(os.path.join('DBSCAN', 'demo'))
    except:
        pass

    try:
        os.mkdir(os.path.join('DBSCAN/demo', args.experiment))
    except:
        pass
    ...
    if args.measurement_model == 'nomo':
        ...
        if False:
            h_w = np.load(
                f'D:\\Francesco Manigrasso\\avatargenerator\\Dataset\\data128\\{args.split}\\dataloaders\\{args.gender}\\test_h_w_measures_{args.gender}_density.npy',
                allow_pickle=True)[0]
            features = np.concatenate([features, np.array(h_w[0]).reshape(1, 1), np.array(h_w[1]).reshape(1, 1)],
                                      axis=-1)
        else:
